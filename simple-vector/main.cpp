...
int main() {
    ...
    SimpleVector<X> vec1(100);
    
    X g(5);
    SimpleVector<X> vec2(100, std::move(g));
    SimpleVector<X> vec3(100, X(5));
    // SimpleVector<X> vec_error(100, g); - данная инструкция вызовет ошибку, поскольку тип X неперемещаемый
    
    int i(5);
    SimpleVector<int> vec4(100, std::move(i));
    SimpleVector<int> vec5(100, int(5));
    SimpleVector<int> vec6(100, i);
    
    SimpleVector<int> vec7 = {1, 2, 3};
    // SimpleVector<X> vec8 = {X(1), X(2), X(3)}; - не знаю, как сделать, чтобы и такой вариант работал
